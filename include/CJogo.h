...
public:
    ...
    CJogo(char *nome,int cursor=0,int altura=0,int largura=0){
        rodando = true;
    ...
    PIG_Evento PegaEvento(){
        ...
        if (SDL_PollEvent((&event))){
            ...
            case SDL_MOUSEMOTION:
                ...
                ultimoEvento.mouse.posY = CGerenciadorJanelas::GetAltura(ultimoEvento.mouse.numeroJanela) - event.motion.y;
                ultimoEvento.mouse.relX = event.motion.xrel;
                ultimoEvento.mouse.relY = -event.motion.yrel;
                if (cursorPadrao!=0)
                    CMouse::Move(ultimoEvento.mouse.posX,CGerenciadorJanelas::GetAltura(ultimoEvento.mouse.numeroJanela)-ultimoEvento.mouse.posY);
                break;
            case SDL_MOUSEWHEEL:
                ultimoEvento.tipoEvento = EVENTO_MOUSE;
    ...
    void DesenhaPonto(int x,int y,PIG_Cor cor,int idJanela=0){
        SDL_SetRenderDrawColor(CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),cor.r,cor.g,cor.b,255);
        SDL_RenderDrawPoint(CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),x,CGerenciadorJanelas::GetAltura(idJanela)-y);
    }
