...
public:
    ...
    static int CriaGeradorParticulas(int maxParticulas,char* nomeArquivoBMP,int idJanela=0,int usaGerenciadorTimer=1){
        int resp = numGeradores->RetiraLivre();
        geradores[resp] = new CGeradorParticulas(maxParticulas,CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),nomeArquivoBMP,usaGerenciadorTimer);
        totalGeradores++;
        return resp;
    }
