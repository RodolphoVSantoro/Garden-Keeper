...
#define PIG_H
/********************************
Versão 0.6.1 da Biblioteca Pig.h
********************************/
...
void CriaJogo(char *nomeJanela,int cursorProprio=0,int altura=0,int largura=0){
    if (jogo==NULL){
...
/*void EscreveFPS(int idJanela=0){
    char fps[32];
    sprintf(fps,"%.2f",GetFPS());
    int altura,largura;
    GetDimensoesJanela(&altura,&largura,idJanela);
    EscreverEsquerda(fps,10,altura-40);
}

void DesenhaPonto(int x, int y, PIG_Cor cor=PRETO, int idJanela=0){
    jogo->DesenhaPonto(x,y,cor,idJanela);
}

#include "extension/iluminacao.h"
#include "extension/GUI.h"
#include "extension/cena.h"
...
int criaPontoLuz(int x, int y, unsigned char intensidade, int raio, unsigned char fadeOut=0, int idJanela=0){
    int idPontoLuz = shadowmap.CriaPontoLuz(x,y,intensidade,raio,fadeOut,idJanela);
    shadowmap.calculaIluminacao(idJanela);
    return idPontoLuz;
}

void movePontoLuz(int x,int y, int idPonto){
    shadowmap.pontosLuz[idPonto].posX=x;
    shadowmap.pontosLuz[idPonto].posY=y;
    shadowmap.calculaIluminacao(shadowmap.pontosLuz[idPonto].idJanela);
}
...
void DesenhaIluminacao(int idJanela=0){
    ...
    SDL_Texture *textura = shadowmap.textura[idJanela];
    SDL_Renderer *renderer = shadowmap.renderer[idJanela];
    //SDL_SetRenderTarget(renderer,textura);
    unsigned char *pixels = (unsigned char*)malloc(sizeof(unsigned char)*shadowmap.altura[idJanela]*shadowmap.largura[idJanela]*4);
    for(int i=0;i<shadowmap.altura[idJanela];i++){
        for(int j=0;j<shadowmap.largura[idJanela];j++){
            const unsigned int offset = ( shadowmap.largura[idJanela] * 4 * i ) + j * 4;
            pixels[offset+0]=0;
            pixels[offset+1]=0;
            pixels[offset+2]=0;
            pixels[offset+3]=255-shadowmap.Mapa[idJanela][i][j];
        }
    }
    SDL_UpdateTexture(textura,NULL,&pixels[0],shadowmap.largura[idJanela]*4);
    free(pixels);
